...
| D4 | Signing | Local `keytool` JKS (RSA 2048); base64 + passwords as GitHub secrets. `client/android/app/build.gradle` release `signingConfig` reads env (`ANDROID_KEYSTORE_PATH/PASSWORD/ALIAS/KEY_PASSWORD`); absent → Gradle fails loudly | Alternative: committed `key.properties` — rejected (secret in git). Offline backup = project action item |
