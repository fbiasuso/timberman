...
- `apk-lts-distribution`: Stable LTS APK URL in a public Supabase Storage bucket (`apk/timberman.apk`, `Cache-Control: no-cache`) as the `/install` download source — GitHub-free install page, README-documented per-release upload (automated CI `upload-lts` job primary; dashboard or Storage API curl with service role key as emergency fallback), `version.txt` object upserted per release and read live by `/install`, GitHub Releases untouched as backup.
- `apk-version-display`: In-app version labels on the splash screen (bottom-center, fades with the overlay) and login/register screens (both tabs), rendered as `v{APP_VERSION}` from the existing central constant (`client/src/constants/app-version.ts` ← `package.json` via `resolveJsonModule`), styled subtly with theme tokens.
